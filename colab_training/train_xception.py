...
def printAndSaveHistoryToFile(history):
    ...
    json.dump(history_dict,
              open(os.path.join(CHECKPOINT_PATH, "history.json"), 'w'))
...
def defineCustomCallbacks():
    log_dir = "/content/gdrive/MyDrive/ColabStuff/xai-deepfake-detection/model_checkpoints/logs/xception2"
    # log_dir = "/content/gdrive/MyDrive/ColabStuff/xai-deepfake-detection/model_checkpoints/logs/fit"
...
# Load checkpoint:
checkpoint_path = os.path.join(CHECKPOINT_PATH,"xception_model.h5")
...
trainCNN(model, train_generator, val_generator, custom_callbacks, initial_epoch)